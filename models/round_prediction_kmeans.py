...
def run_f1_across_trials(model_name, simulate_fn, n_trials=10):
    kmeans = joblib.load(f'kmeans_{model_name}_model.pkl')
    mapping = joblib.load(f'kmeans_{model_name}_mapping.pkl')
    num_players = 12
    num_colluders = 3
    ...
    for trial in range(n_trials):
        save_dir = f"live_kmeans_{model_name}_trial_{trial}"
        os.makedirs(save_dir, exist_ok=True)
        simulate_fn(save_dir=save_dir, num_players=num_players, num_colluders=num_colluders)
        ...
        for t in range(1, max_rounds):
            X_t = extract_features_up_to_t(rewards, regrets, t)
            preds = np.array([mapping[c] for c in kmeans.predict(X_t)])
            f1 = f1_score(true, preds)
            f1_list.append(f1)
        f1_matrix.append(f1_list)

    return np.arange(1, max_rounds), np.array(f1_matrix)  # (T,), (n_trials, T)
...
for model_name, sim_fn in {
    ...
    times, f1_runs = run_f1_across_trials(model_name, sim_fn, n_trials=n_trials)
...
plt.xlabel("Round")
plt.ylabel("F1 Score")
#plt.title(f"KMeans Collusion Detection\nAverage F1 Score ± Std Across {n_trials} Simulations")
plt.ylim(0, 1.05)
plt.xlim(0, 300)
plt.grid(True)
plt.legend()
